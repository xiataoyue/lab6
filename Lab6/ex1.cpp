...
int main() {
    ...
    string str;        //To get input line
    int num_node;      //The first line------num_nodes
    string name;       //the name of node
    int num_child;     //The number of children of the node
    int temp_num;      //used for some children
    string temp_name;     //used for some children
    ...
    for(int i = 0; i < num_node; i++) {
        ...
        Istream.str(str);
        Istream >> name >> num_child;

        if(!name.compare("ProgramNode")) {
            nodes[i] = new ProgramNode(num_child);
        }
        else if(!name.compare("DeclListNode")) {
            nodes[i] = new DeclListNode(num_child);
        }
        else if(!name.compare("FormalsListNode")) {
            nodes[i] = new FormalsListNode(num_child);
        }
        else if(!name.compare("FnBodyNode")) {
            nodes[i] = new FnBodyNode(num_child);
        }
        else if(!name.compare("StmtListNode")){
            nodes[i] = new StmtListNode(num_child);
        }
        else if(!name.compare("VarDeclNode")) {
            nodes[i] = new VarDeclNode(num_child);
        }
        else if(!name.compare("FnDeclNode")) {
            nodes[i] = new FnDeclNode(num_child);
        }
        else if(!name.compare("FormalDeclNode")) {
            nodes[i] = new FormalDeclNode(num_child);
        }
        else if(!name.compare("IntNode")) {
            nodes[i] = new IntNode(num_child);
        }
        else if(!name.compare("BoolNode")) {
            nodes[i] = new BoolNode(num_child);
        }
        else if(!name.compare("VoidNode")) {
            nodes[i] = new VoidNode(num_child);
        }
        else if(!name.compare("AssignStmtNode")) {
            nodes[i] = new AssignStmtNode(num_child);
        }
        else if(!name.compare("PostIncStmtNode")) {
            nodes[i] = new PostIncStmtNode(num_child);
        }
        else if(!name.compare("PostDecStmtNode")) {
            nodes[i] = new PostDecStmtNode(num_child);
        }
        else if(!name.compare("IntLitNode")) {
            Istream >> temp_num;
            nodes[i] = new IntLitNode(num_child, temp_num);
        }
        else if(!name.compare("TrueNode")) {
            nodes[i] = new TrueNode(num_child);
        }
        else if(!name.compare("FalseNode")) {
            nodes[i] = new FalseNode(num_child);
        }
        else if(!name.compare("IdNode")) {
            Istream >> temp_name;
            nodes[i] = new IdNode(num_child, temp_name);
        }
        else if(!name.compare("AssignNode")) {
            nodes[i] = new AssignNode(num_child);
        }
        else {
            cout << "No matching node name!" << endl;
            return 0;
        }
    }
    ...
    for(int i = 0; i < num_node; ++i) {         //To skip Program node for it only contains one child
        for(int j = 0; j < nodes[i]->numChild(); ++j) {
            nodes[i]->setChild(j, nodes[count + 1]);
            count ++;
        }
    }
